...
def __main__():
    ...
    train_feat_tensor, test_feat_tensor, _, train_label_tensor, test_label_tensor, test_result_path= get_data_dict(env_index, config_file, current_base_path, data_postfix, "test", return_path = "result_path", prep_baseline = True, cluster= "tetralith")
    train_feat_tensor, genz_feat_tensor, _, train_label_tensor, genz_label_tensor, genz_result_path= get_data_dict(env_index, config_file, current_base_path, data_postfix, "genz", return_path = "result_path", prep_baseline = True, cluster= "tetralith")
    ...
    assert train_feat_tensor.size(-1) == genz_feat_tensor.size(-1), "Feature dimension mismatch"
    # assert train_label_tensor.size(-1) == genz_label_tensor.size(-1), "Label dimension mismatch"
    ...
    genz_dataset = TensorDataset(genz_feat_tensor, genz_label_tensor)
    if env_index < 50:
        gendiag_dataset = TensorDataset(gendiag_feat_tensor, gendiag_label_tensor)
    # import ipdb; ipdb.set_trace()
    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False)
    genz_loader = DataLoader(genz_dataset, batch_size=16, shuffle=False)
    ...
    # Test the model
    model.eval()
    loader_dict = {"test": test_loader, "genz": genz_loader}
    path_dict = {"test": test_result_path, "genz": genz_result_path}
    if env_index < 50:
        loader_dict["gendiag"] = DataLoader(gendiag_dataset, batch_size=16, shuffle=False)
    ...
    for test_data_name, test_loader in loader_dict.items():
        ...
        with torch.no_grad():
            for batch_test_data, batch_test_labels  in test_loader :
                ...
                list_MLP_RSSI_all_gts.append(batch_test_labels[...,0].cpu().numpy())
                ...
                list_MLP_RSSI_pred.append(outputs[...,0].cpu().numpy())
        ...
        # save the model
        torch.save(model.state_dict(), f"{path_dict[test_data_name]}/MLP_model_{test_data_name}_{running_datetime_str}.pth")
