...
template = """#!/bin/bash
#!/bin/bash
#SBATCH -J test_dt_wonoise_wType_long{env_id}
#SBATCH -n 1
#SBATCH --gpus 1
#SBATCH -t 1-14:59:59
#SBATCH --mail-type=ALL
#SBATCH --mail-user=yifeij@kth.se
#SBATCH -o ./test_dt_wonoise_wType_long{env_id}_full.out
#SBATCH -e ./test_dt_wonoise_wType_long{env_id}_full.err
...
python test_dt.py --allTx True --env_index {env_id} --eval_from latest --test_data test  --add_type_loss True
python test_dt.py --allTx True --env_index {env_id} --eval_from latest --test_data genz  --add_type_loss True
python test_dt.py --allTx True --env_index {env_id} --eval_from latest --test_data gendiag  --add_type_loss True

"""

# Create directory for the SLURM scripts if it does not exist
output_dir = 'gen_test_scripts_wonoise_wType'
